...
my $ctrl_flg_command_tgt_id;          #which command(BL/CN) will be excuted based on the given number.>0, BL; <=0 CN & Qn
...
if($flag == 1)
{
  ...
  elsif($ctrl_flg_command_tgt_id==0){$flg_BL=0;}
  else{$flg_BL=-1;}
...
#see L919      {;}
  ...
  while($dir_loop_i<=$sampling_indx_end_dir)
  {
    ...
    @values=split(/\r|\n/,$line);chomp(@values);
    $size_frame=$total_line/($#values+1);
#$_=17345.3458;
    ...
    #11/13/2018 program can scan different depth from the first target atom
      ...
      { 
        ...
        if($dir_loop_i == $sampling_indx_start_dir and $dir_loop_i == $sampling_indx_end_dir)
        {
      ...
      while ($line=<TMP_FILE>)
      {
        ...
        {#convert fractional coordinates to absolute coordinates
          ...
          { 
            ...
            {
              $new_aaa=$aaa;$new_bbb=$bbb;$new_ccc=0;   #a,b direction will be duplicated periodically, boader along c will be extendeda
              ...
              { 
                ...
                if($tmp eq 'O'){$cnt_oxyg_intf++;if($atom_id <= $atm_amt_glass){$oxy_gls_intf_cnt++;}else{$oxy_wtr_intf_cnt++;}}
                elsif($tmp eq 'H'){$cnt_hydg_intf++;}
                #elsif($tmp eq "CA"){$cnt_modf_intf++;$hash_modf_intf_indv_amt{$tmp}++;}
                elsif(grep { $tmp eq $_ } @former_lists){$cnt_former_intf++;$hash_former_intf_indv_amt{$tmp}++;} 
                elsif(grep { $tmp eq $_ } @modifer_lists){$cnt_modf_intf++;$hash_modf_intf_indv_amt{$tmp}++;}
                else{$cnt_other_intf++;}
              ...
              { 
                ...
                { 
                  ...
                  if($tmp eq 'O'){$cnt_oxyg_exbox++;if($atom_id <= $atm_amt_glass){$oxy_gls_exbox_cnt++;}else{$oxy_wtr_exbox_cnt++;}}
                  elsif($tmp eq "H"){$cnt_hydg_exbox++;}
                  elsif(grep { $tmp eq $_ } @former_lists){$cnt_former_exbox++;$hash_former_exbox_indv_amt{$tmp}++;} 
                  elsif(grep { $tmp eq $_ } @modifer_lists){$cnt_modf_exbox++;$hash_modf_exbox_indv_amt{$tmp}++;}
            ...
            elsif($flg_interface==-2 or $flg_interface==2)           #extend along y direction
            {
              ...
              { $flg_intf_atm[$atom_id]=1;
                ...
                #elsif(grep { $tmp eq $_ } @former_lists == 1){$cnt_former_intf++;$hash_former_intf_indv_amt{$tmp}++;} 
                #elsif(grep { $tmp eq $_ } @modifer_lists == 1){$cnt_modf_intf++;$hash_modf_intf_indv_amt{$tmp}++;}
                elsif(grep { $tmp eq $_ } @former_lists ){$cnt_former_intf++;$hash_former_intf_indv_amt{$tmp}++;} 
                elsif(grep { $tmp eq $_ } @modifer_lists ){$cnt_modf_intf++;$hash_modf_intf_indv_amt{$tmp}++;}
                else{$cnt_other_intf++;}
              }
            ...
            }#end elsif($flg_interface==-2 or $flg_interface==2)
            ...
            }#end else of elsif($flg_interface==-1 or $flg_interface==1)
            else
            {;}
...
### tmp pend                $tmp=uc($hash_type_spc{$hash_key});chomp($tmp);
### tmp pend                if($tmp eq 'O'){$cnt_oxyg_intf++;if($atom_id <= $atm_amt_glass){$oxy_gls_intf_cnt++;}else{$oxy_wtr_intf_cnt++;}}
### tmp pend                elsif($tmp eq 'H'){$cnt_hydg_intf++;}
### tmp pend                #elsif($tmp eq "CA"){$cnt_modf_intf++;$hash_modf_intf_indv_amt{$tmp}++;}
### tmp pend                elsif(grep { $tmp eq $_ } @former_lists){$cnt_former_intf++;$hash_former_intf_indv_amt{$tmp}++;} 
### tmp pend                elsif(grep { $tmp eq $_ } @modifer_lists){$cnt_modf_intf++;$hash_modf_intf_indv_amt{$tmp}++;}
...
### tmp pend                $tmp=uc($hash_type_spc{$hash_key});chomp($tmp);
### tmp pend                
### tmp pend                if($tmp eq 'O'){$cnt_oxyg_exbox++;if($atom_id <= $atm_amt_glass){$oxy_gls_exbox_cnt++;}else{$oxy_wtr_exbox_cnt++;}}
### tmp pend                elsif($tmp eq "H"){$cnt_hydg_exbox++;}
### tmp pend                elsif(grep { $tmp eq $_ } @former_lists){$cnt_former_exbox++;$hash_former_exbox_indv_amt{$tmp}++;} 
### tmp pend                elsif(grep { $tmp eq $_ } @modifer_lists){$cnt_modf_exbox++;$hash_modf_exbox_indv_amt{$tmp}++;}
...
### tmp pend              { $flg_intf_atm[$atom_id]=0;}
      ...
      else{$search_atom_id=1;$search_loop_limit=$atm_amt;$search_depth=1;}
      ...
      {
#print "1234 \$search_atom_id=$search_atom_id\n";
        # here is complicated.
        ...
        if($flg_intf_atm[$search_atom_id]==0 and $flg_BL==0)   #search_atom is not in the interface box and not calculate BL, skip it
        {$search_atom_id++;next;}
        else
        {
#if($flg_intf_atm[$search_atom_id]==0){print"1042 \$flg_intf_atm[$search_atom_id]=$flg_intf_atm[$search_atom_id]\n";}
            ### reset parameters
...
#print "1048 \@flg_neighbor_level_",@flg_neighbor_level,"\n";$_=<STDIN>;
            @flg_neighbor_level=();
...
#if(uc($tgt_nm) eq 'H'){print "1225 H=$tgt_nm\_$search_atom_id \$flg_intf_atm[$search_atom_id]=$flg_intf_atm[$search_atom_id]\t\$cn_no=$cn_no\n";$_=<STDIN>;}
#print colored("1119 ck main search_atom_id= $tgt_nm\_$search_atom_id\tcn=$cn_no\t\$flg_intf_atm[$search_atom_id]=$flg_intf_atm[$search_atom_id]\n","bold magenta");
            #if(grep(/$tgt_nm/,@former_lists))                         #atom name is in former list
            if(grep { $tgt_nm eq $_ } @former_lists)                         #atom name is in former list
            { 
#for($tmp=0;$tmp<=8;$tmp++){print colored("1122 tot_cn($tmp)=$hash_former_cn_tot_amt{$tmp}\ttot_cn_exbox($tmp)=$hash_former_exbox_cn_tot_amt{$tmp}\n","red");}
              $flg_former=1;
...
#print colored("1128 \$hash_former_cn_tot_amt{$cn_no}=$hash_former_cn_tot_amt{$cn_no}\t\$hash_former_exbox_cn_tot_amt{$cn_no}=$hash_former_exbox_cn_tot_amt{$cn_no}\n","bold yellow");
#for($tmp=0;$tmp<=8;$tmp++){print colored("1129 tot_cn($tmp)=$hash_former_cn_tot_amt{$tmp}\ttot_cn_exbox($tmp)=$hash_former_exbox_cn_tot_amt{$tmp}\n","red bold");}
#$tmp=<STDIN>;
            }#end 
            elsif(uc($tgt_nm) eq 'O')                                 #atom name is oxygen
            { 
#for($tmp=0;$tmp<=8;$tmp++){print colored("1134 tot_cn($tmp)=$hash_oxygen_cn_tot_amt{$tmp}\t\$hash_oxygen_cn_glass_tot_amt($tmp)=$hash_oxygen_cn_glass_tot_amt{$tmp}\t\$hash_oxygen_cn_water_tot_amt{$tmp}=$hash_oxygen_cn_water_tot_amt{$tmp}\n","red");}
              $flg_former=0;
...
#print colored("1158 \$hash_oxygen_cn_tot_amt{$cn_no}=$hash_oxygen_cn_tot_amt{$cn_no}\t\$hash_oxygen_exbox_cn_tot_amt{$cn_no}=$hash_oxygen_cn_exbox_tot_amt{$cn_no}\n","bold yellow");
#for($tmp=0;$tmp<=8;$tmp++){print colored("1159 tot_cn($tmp)=$hash_oxygen_cn_tot_amt{$tmp}\t\$hash_oxygen_cn_glass_tot_amt($tmp)=$hash_oxygen_cn_glass_tot_amt{$tmp}\t\$ hash_oxygen_cn_glass_exbox_tot_amt{$tmp}= $hash_oxygen_cn_glass_exbox_tot_amt{$tmp}\t\$hash_oxygen_cn_water_tot_amt{$tmp}=$hash_oxygen_cn_water_tot_amt{$tmp}\t\$hash_oxygen_cn_water_exbox_tot_amt{$tmp}=$hash_oxygen_cn_water_exbox_tot_amt{$tmp}\n","red");}
#$tmp=<STDIN>;
            }#end elsif(uc($tgt_nm) eq 'O')
            #elsif(grep(/$tgt_nm/,@modifer_lists))
            elsif(grep { $tgt_nm eq $_ } @modifer_lists)
            {
#for($tmp=0;$tmp<=8;$tmp++){print colored("1165 tot_cn($tmp)=$hash_modifier_cn_tot_amt{$tmp}\t\$ hash_modifier_cn_exbox_tot_amt{$tmp}= $hash_modifier_cn_exbox_tot_amt{$tmp}\n","red");}
              $flg_former=0;
...
#print colored("1171 \$hash_modifier_cn_tot_amt{$cn_no}=$hash_modifier_cn_tot_amt{$cn_no}\t\$hash_modifier_cn_exbox_tot_amt{$cn_no}=$hash_modifier_cn_exbox_tot_amt{$cn_no}\n","bold yellow");
#for($tmp=0;$tmp<=8;$tmp++){print colored("1172 tot_cn($tmp)=$hash_modifier_cn_tot_amt{$tmp}\t\$ hash_modifier_cn_exbox_tot_amt{$tmp}= $hash_modifier_cn_exbox_tot_amt{$tmp}\n","red");}
#$tmp=<STDIN>;
            }
...
#if($search_nghb_id==11430 or $search_nghb_id==10554){print colored("1042: \$zzz[$search_nghb_id]=$zzz[$search_nghb_id]\t\$search_level=$search_level\t\$flg_intf_atm[$search_nghb_id]=$flg_intf_atm[$search_nghb_id]\n","bold white");$tmp=<STDIN>;}
                $line=&cal_dist($search_nghb_id,$flg_intf_atm[$search_atom_id]);$line=$line=tm::trim($line);
#print "1044 \$line=$line\n";    
...
OUTPUT_CN_QN:
  ...
  {
    if($flg_interface>0){$file_cn_qn="$work_dir/$ctrl_dir_trck/cb$comp_id\_CN_Qn_size_$sampling_size\_TMSTP_$frm_time\_$sampling_start_timestep_no-$sampling_end_timestep_no.dat";}
    else{$file_cn_qn="$work_dir/$ctrl_dir_trck/cs$comp_id\_CN_Qn_size_$sampling_size\_TMSTP_$frm_time\_$sampling_start_timestep_no-$sampling_end_timestep_no.dat";}
