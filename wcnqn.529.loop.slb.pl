...
# Last modified:	2019-06-09 23:02
# Filename:				wcnqn.529.loop.pl
...
##2019-06-08 11:19 this program missed the last frame when perform CN/Qn distribution with limited frames
##                  so far the results look good for step=500,000fs, limits=1,000,025fs
##2019-10-06 21:47 cannot tell over- and/or under-coordinated formers by layers
...
my $SCRIPT_VERSION="v2019-10-06-16-25";

#constant
my $CONSTANT_SIZE_FRAME=0;       #will be reset after first scanning and will not be changed anymore
...
my  $file_his;                        #history file which include updated species name, eg. La, Th, ...
...
my $ctrl_starting_time_sampling;      #READ following notation for next three variables!!!!!!for CN/Qn distribution, will be converted into frame number
my $ctrl_ending_time_sampling;        #for CN/Qn distribution, will be converted into frame number, for CN/Qn with limited frames, it can be same to $ctrl_starting_time_sampling, and the sample size would be 1; for CN/Qn with a certain amount of frames, it can be larger than $ctrl_starting_time_sampling, BUT $ctrl_loop_tstp_sampling MUST BE 0 and $ctrl_loop_max_time_sampling=$ctrl_ending_time_sampling
...
my $ctrl_flg_stop;                    #flag for STOP/RESUME. default: 0 (no STOP); 1 (there is STOP). If value is 1, search STOP file to get the directory suspended
my $dir_resume;                       #save the breakpoint, when this program continues, it will start from the $dir_resume instead of $dirs[0]
my $indx_dir_resume;                  #save index of array @dirs, the value of $dirs[$indx_dir_resume]==$$dir_resume
...
my $sample_starting_t;                #varizble to save a starting time in a sample
my $sample_ending_t;                  #variable to save an ending time in the same sample
...
my $flg_header=1;                       #1: output header to output file; 0: not output header to output file
...
my %hash_no_former_cn_slb;              #hash table save counted species in each slab. e.g $no_former_cn_slb{ 3-Si-z1},
...
if($flag == 1)
{
  ...
  $ctrl_flg_stop = $sub_values[21];               #flag for STOP/RESUME. default: 0 (no STOP); 1 (there is STOP). If value is 1, search STOP file to get the directory suspended
print "495 \$ctrl_ending_time_sampling=$ctrl_ending_time_sampling\n";#<STDIN>;

  ### set variables based on read parameters above
  if($ctrl_flg_command_tgt_id>0)      # track target atom id, analyze bondlength for 1st, 2nd,..., order neighbors
  { 
  ...
  elsif($ctrl_flg_command_tgt_id==0)  # CN/Qn analysis
  { $flg_BL=0;
  ...
  else{$flg_BL=-1;}                 # CN/Qn distribution, output all info(CN,Qn,BL,neighbors) for each atom in the interface box
  ...
  else
  {
    if($flg_BL ==1)
    { ### output file name
      # Bondlength analysis cost too much time. In case $output_bondlength_file being overwritten, this file should be renamed
  ...
  }#end else of if( !-d "$work_dir/$ctrl_dir_trck" )
  ...
  RESUMEPOINT:  #ONLY FOR BL analysis
  my $file_STOP="STOP";my $file_RESUME="RESUME";
  if($ctrl_flg_stop==1 and $flg_BL==1)   # for BL analysis, if it is interrupted, it can be resume. Not for CN/Qn analysis
  { if(! -e $file_STOP and ! -e $file_RESUME){print "Flag file ",colored("$file_STOP","BOLD YELLOW")," does NOT exist. It might be owing to improper setting in ",colored("$input_control_file","bold cyan")," Pls check flag for $file_STOP. Quit.\n";$flag=0;$error_msg="$error_msg; Flag file $file_STOP does not exist. It might be owing to improper setting in $input_control_file, check flag for STOP";}
    else
    {
      if(-e $file_STOP and -e $file_RESUME){tm::rename($file_RESUME);}   #backup RESUME file in case of overwriten
      if(-e $file_STOP){system("mv $file_STOP $file_RESUME");}
      $_=`grep $target_atom_id $file_RESUME`; $_=tm::trim($_);      #search target atom ID is in $file_RESUME or not 
      if(length($_)>0)      # non empty line
      { $flag=1;@_=split(/\s+/,$_);@values=grep{/$ctrl_sim_dir_keyword/} @_;
        if($#values>0)
        { print "Error! There are more than one values, please check your $file_RESUME file to remove redundant lines.Original record in $file_RESUME:$_ \n\@values=",@values," Quit program.\n";
          $flag=0;
          $error_msg="$error_msg There are more than one line including target atomID in $file_RESUME file. Remove redundant lines. Original record in $file_RESUME:$_;\n";
        }#end if($#values>0)
        else
        { print "Will resume from BREAKPOINT $values[0].\n";$dir_resume=$values[0];}
      }#end if(length($_)>0)
      else{print"The directory for resuming the BL analysis does not exist.Quit.\n";$flag=0;}
      #$sn_frame=14500000;
    }#end else of if(! -e $file_STOP and ! -e $file_RESUME)
  }#end if($ctrl_flg_stop==1)
...
print "634:\@dirs",@dirs,"\n";  #<STDIN>;
  ...
  if ($ctrl_flg_stop==1 and $flg_BL==1)
  { $i=0;
    while($dir_resume ne $dirs[$i])
    {$i++;}
    print "Will resume BL analysis from $dirs[$i]\n";
    $indx_dir_resume=$i;
  }#end if ($ctrl_flg_stop==1)
  ### END BREAKPOINT
print "649:\@dirs",@dirs,"\n";      #<STDIN>;
  ...
  #        print "\$hash_cutoff{$hash_spc_type{$values[0]}-$hash_spc_type{$values[1]}}=$values[2]\n";
    ...
    if ($up_g==-1 or $up_w==-1 or $lo_g==-1 or $lo_w==-1)
    {
      ...
      else
      {
        ...
        #initialize %hash_no_former_cn_slb
        for(my $ss=int($lo_g);$ss<=(int($up_g)+1);$ss++)
        { $tmp=$ss+.5;
#print"868: z=$tmp\n";<>;
          for($cn_no=-1;$cn_no<=$ctrl_max_cn;$cn_no++)
          {
            foreach(@former_lists)
            { $hash_key="$cn_no-$_-$ss";    #hash key for formers
              $hash_no_former_cn_slb{$hash_key}=0;
            }#for($cn_no=-1;$cn_no<=$ctrl_max_cn;$cn_no++)
          
            $hash_key="$cn_no-O-$ss";    #hash key for formers
            $hash_no_former_cn_slb{$hash_key}=0;

          }#foreach(@former_lists)
        }#for(my $ss=int($up_g)+1;$ss>=(int($lo_g)-1);$ss--)
...
print"879 \@dirs:",@dirs,"\n";#<STDIN>; 
  ...
  else
  {
    ...
    else{print "941 no change \$ctrl_ending_time_sampling=$ctrl_ending_time_sampling\n";}   #<STDIN>;}
...
#del $_=<STDIN>;
  ...
  ################################################################################
  SETTING_SAMPLE_RANGE_OR_TIME_RANGE:
print"1011: \$flg_BL=$flg_BL\n";#<STDIN>;
  if($flg_BL<0)     #set @sampling_t_lists. CN/Qn analysis for limited amount of frames because of search depth=2
  {
    $j=0;
    $sample_starting_t=$ctrl_starting_time_sampling;
    $sample_ending_t=$ctrl_ending_time_sampling;
    while ($sample_ending_t<=$ctrl_loop_max_time_sampling)
    {
      push(@sampling_t_lists,$sample_starting_t,$sample_ending_t);
      if($j<20){print"983 j=$j\tstarting_time_sample=\$sampling_t_lists[$j]=$sampling_t_lists[$j]\tending_time_sample=\$sampling_t_lists[$j+1]=$sampling_t_lists[$j+1]\t\$time_step=$time_step\t\$dump_gap=$dump_gap\n";} #$tmp=<STDIN>;
      else{last;}
      $j+=2;
      $sample_starting_t=$ctrl_starting_time_sampling+$j/2*$ctrl_loop_tstp_sampling;  #$j/2 results from $j+=2 which increase step value double
      $sample_ending_t=$ctrl_ending_time_sampling+$j/2*$ctrl_loop_tstp_sampling;
    }#end while ($_<=$ctrl_loop_max_time_sampling)
  }#end if($flg_BL<0)     ##set @sampling_t_lists. CN/Qn analysis for limited amount of frames
  elsif($flg_BL==0)      # CN/Qn analysis for a certain amount of frames, search depth=1
  {   #here suppose $ctrl_ending_time_sampling is very different from $ctrl_starting_time_sampling
      push(@sampling_t_lists,$ctrl_starting_time_sampling);
      if($ctrl_ending_time_sampling != $ctrl_loop_max_time_sampling)
      { $warn_msg="$warn_msg Regular CN/Qn analysis request ending time of a sample is same to the max time of a sample unless you want to perform CN/Qn analysis with limited frames. Please modify the values (Line#:18 and 20) in input_control.in. Quit Program;";
        $flag=0;
        goto MAIN;
      }
      else
      {  push(@sampling_t_lists,$ctrl_ending_time_sampling);}
  }#end elsif($flg_BL==0) # CN/Qn analysis for a certain amount of frames
  else    #BL analysis
  {   
      ### if the program starts from a breakpoint/BREAKPOINT, 
      ### the starting sample time should be updated so user does not need to calculate it
      if($ctrl_flg_stop==0){$sample_starting_t=0;$sn_frame=0;}  #for starting sample when BL analysis performed without BREAKPOINT
      else #start from a BREAKPOINT
      {
        $tmp="$work_dir/$dir_resume/$ctrl_anly_dir/$ctrl_dump_struc_file";     #1st resuming file
        $sn_frame=`head -n 2 $tmp|tail -n 1`;$sn_frame=tm::trim($sn_frame);        #TIMESTEP in dump file
        $sn_frame=$sn_frame/$dump_gap;
        $sample_starting_t=$sn_frame*$time_step*$dump_gap;       #convert TIMESTEP into time
print "1056: \$sn_frame=$sn_frame \$ctrl_loop_tstp_sampling=$ctrl_loop_tstp_sampling\$sample_starting_t=$sample_starting_t\n";#<STDIN>;
      }#end else of if($ctrl_flg_stop==0)
      push(@sampling_t_lists,$sample_starting_t); # initial resuming time
      push(@sampling_t_lists,$time_max);    #maximum timesteps which value shows in the last frame of the last dirs. e.g. TIMESTEP in 20mstp, where 20mstp is the last dirs
  }#end else of elsif($flg_BL==0) of if($flg_BL<0)
  END_SETTING_SAMPLE_RANGE_OR_TIME_RANGE:

if($flg_BL==1){print "1041 \@sampling_t_lists: ",@sampling_t_lists,"\n\$_=$_\n\$sample_starting_t=$sample_starting_t\n\$sn_frame=$sn_frame\n\$ctrl_loop_tstp_sampling=$ctrl_loop_tstp_sampling\n";} #$_=<STDIN>;}
################################################################################
############## read lammpstrj and convert coordinates
  print "Counting frame number and line number. Please wait ...\n";
  chomp@line_no;

print "1064: \$#sampling_t_lists=$#sampling_t_lists\ti\@sampling_t_lists=",@sampling_t_lists,"\n";#<STDIN>;
print "1065 \$ctrl_ending_time_sampling=$ctrl_ending_time_sampling\n";#<STDIN>; 
#  SETTINGS_DIR_TIME:
  for($sampling_t_loop_i=0;$sampling_t_loop_i<$#sampling_t_lists;$sampling_t_loop_i=$sampling_t_loop_i+2) #NOTE, even index of $sampling_t_lists is for time of starting sample, odd index is for time of ending sample
  { 
    ## Here, the settings for $ctrl_starting_time_sampling and $ctrl_ending_time_sampling are crutial
    ## if for CN/Qn with limited frames, both are same, namely ONLY 1 frame as a sample
    ## if for CN/Qn with a certain amout of frames,get the lower and upper limit values
    ## if for BL, get the whole range from 1 to the maximum value
    $ctrl_starting_time_sampling=$sampling_t_lists[$sampling_t_loop_i];
    $ctrl_ending_time_sampling=$sampling_t_lists[$sampling_t_loop_i+1];
print"1080: \$ctrl_starting_time_sampling=$ctrl_starting_time_sampling\n";
...
print "1100 \$ctrl_ending_time_sampling=$ctrl_ending_time_sampling\n\$sampling_start_timestep_no=$sampling_start_timestep_no\t\$sampling_start_frame_no=$sampling_start_frame_no\n\$sampling_end_timestep_no=$sampling_end_timestep_no\t\$sampling_end_frame_no=$sampling_end_frame_no\n";#<STDIN>; 
    ## calculation of data location (in which directory) is complicated
    ## it goes through  $sampling_start_timestep_no, $sampling_start_frame_no, $sampling_end_timestep_no, and $sampling_end_frame_no
    ## the calculated $sampling_start_frame_no and $sampling_end_frame_no may be the last or the first frame in a directory
    ## let say the starting frame is the last frame in the directory 1mstp and the ending frame is in the directory 1.5mstp
    ## after using following equation to calculate the frame number, however, the index for an array starting from 0
    ## so, if remainder is 0, the index should minus 1
    ## samre for the ending dir calculation
    
    #here, the calculation for sampling size is not clear
    #if take the 1st frame, and end the 1st frame, only 1 frame (sample)
    #if take the 1st frame, and end the 2nd frame, 2 frames (samples)
    #...
    #apparently, the end one is always included
    #it might confuse users when set the starting and ending time if set start and end at the same frame, the calculation is 0
    #but the setting makes sense
    #this program use this way to calculate it. Setting files should clearly indicate this
    $sampling_size=($sampling_end_timestep_no-$sampling_start_timestep_no+$dump_gap)/$dump_gap;   #not accurate when sample is 1. have not tested

    # Here it is a little confused and tricky, so give an example below. 
    # timestep=.25fs, dump 1frame/100timestep, total time 5ns (20000000 TIMESTEPS), each dump file has 500,000 TIMESTEPS (~125,000fs)
    # e.g. if start from 7.5mstp, the first TIMESTEP=7000000 (t=1750000fs); in 7mstp, the last TIMESTEP=7000000 as well
    # general case
    # 1. $_=$sampling_start_timestep_no%$run_no_timestep == 0 or $_=$sampling_end_timestep_no%$run_no_timestep == 0 ==> at beginning or ending TIMESTEP 
    #   1.1 if $ctrl_starting_time_sampling=0,for starting time, it starts from 500kstp
    #     if $ctrl_ending_time_sampling=0, for ending time, it ends by 500kstp
    #   1.2 if $ctrl_starting_time_sampling=1750000, for starting time, it can start from 7.5mstp, 
    #     but for ending time, it should end by 7mstp
    #   1.3 if $ctrl_starting_time_sampling=5,000,000fs (5ns), it starts from 20mstp
    #     for ending time, it should end by 20mstp
    # 2. $_=$sampling_start_timestep_no%$run_no_timestep != 0 or $_=$sampling_end_timestep_no%$run_no_timestep != 0 
    #   2.1 if $ctrl_starting_time_sampling + $run_no_timestep < $ctrl_ending_time_sampling
    #       $ctrl_starting_time_sampling= 100 (fs) (TIMESTEP 400), $ctrl_ending_time_sampling = 150,000 (fs) (TIMESTEP 600,000)
    #       starting from 500kstp                   ending by 1mstp
    #       400%500,000=400, int(400/500,000)=0   600,000%500,000=100,000 int(600,000/500,000)=1
    #   2.2 if $ctrl_starting_time_sampling + $run_no_timestep = $ctrl_ending_time_sampling
    #       100 (fs) (TIMESTEP 400)         + 125,000 (fs) (~500,000)=126,000(fs) (TIMESTEP=500,400)
    #       500kstp                                                       1mstp
    #       0 (fs) (TIMESTEP 0)             + 125,000 (fs) (~500,000)=125,000(fs) (TIMESTEP=500,000)
    #       500kstp                                                       500kstp
    #   2.3 if $ctrl_starting_time_sampling + $run_no_timestep > $ctrl_ending_time_sampling
    #       0 (fs) (TIMESTEP 0)             + 125,000 (fs) (~500,000)>120,000(fs) (TIMESTEP=480,000)
    #       500kstp                                                       500kstp
 
print "1108: \$sampling_start_timestep_no=$sampling_start_timestep_no\t\$run_no_timestep=$run_no_timestep\n";#<STDIN>;    
    $sampling_indx_start_dir=int($sampling_start_timestep_no/$run_no_timestep);
    $_=$sampling_start_timestep_no%$run_no_timestep;

    print "\$sampling_t_lists[$sampling_t_loop_i]=$sampling_t_lists[$sampling_t_loop_i]\t\$ctrl_starting_time_sampling=$ctrl_starting_time_sampling\t\$ctrl_ending_time_sampling=$ctrl_ending_time_sampling\n\$sampling_start_timestep_no=$sampling_start_timestep_no\t\$sampling_start_frame_no=$sampling_start_frame_no\n\$sampling_end_timestep_no=$sampling_end_timestep_no\t\$sampling_end_frame_no=$sampling_end_frame_no\n\$sampling_size=$sampling_size\nremainder=$sampling_start_timestep_no%$run_no_timestep=$_\n\$sampling_indx_start_dir=$sampling_indx_start_dir\t";#$tmp=<STDIN>;

   
    #??? logic confusion
    #if($_==0 and $ctrl_starting_time_sampling>0){$sampling_indx_start_dir--;}
    if($ctrl_starting_time_sampling >= $time_max)    #if starting time is greater than maximum simulating time, here > 5ns
    { 
      #$flag=0;
      $warn_msg="$warn_msg starting time of sampling is greater than $time_max(fs).Change starting time from $ctrl_starting_time_sampling to $time_max;";
      print "Error! Starting time is greater than the maximum time $time_max (fs). Change starting time from        $ctrl_starting_time_sampling to $time_max;\n";        
      $ctrl_starting_time_sampling = $time_max;
      $sampling_indx_start_dir=$#dirs;
    }#end if($ctrl_starting_time_sampling>$time_max)          
    elsif($ctrl_starting_time_sampling< $time_min)
    { #$flag=0;
      $warn_msg="$warn_msg starting time of sampling is less than $time_min(fs). Change starting time from $ctrl_starting_time_sampling to $time_min;\n";
      print "Error! Starting time is less than the minimum time $time_min (fs). Change starting time from                            $ctrl_starting_time_sampling to $time_min.\n";
      $ctrl_starting_time_sampling = $time_min;
      $tmp=int($time_min/$run_no_timestep);
      $j=0;
      while($dirs[$j] ne $tmp){$j++;}
      $sampling_indx_start_dir=$j;
    }#end elsif($ctrl_starting_time_sampling<0)
    else # others, do nothing
    {;}#end else of elsif($ctrl_starting_time_sampling< $time_min) of if($ctrl_starting_time_sampling>$time_max)

    $sampling_indx_end_dir=int($sampling_end_timestep_no/$run_no_timestep);
    $_=$sampling_end_timestep_no%$run_no_timestep;
    
    print "final value= $sampling_indx_start_dir\t$dirs[$sampling_indx_start_dir]\n\$sampling_indx_end_dir=$sampling_indx_end_dir\t";#$tmp=<STDIN>;

    # ??? logic confusion
    #if($_==0 and $ctrl_ending_time_sampling>0){$sampling_indx_end_dir--;}
    if($ctrl_ending_time_sampling > $time_max)    #if $ctrl_ending_time_sampling is greater than maximum simulating time, here > 5ns
    {
      print "Error! Ending time is greater than the maximum time $time_max (fs). Change ending time from                          $ctrl_ending_time_sampling to $time_max;\n";
      $ctrl_ending_time_sampling = $time_max;
      $sampling_indx_end_dir=$#dirs;
    }#end if($ctrl_ending_time_sampling > $time_max)
    elsif($ctrl_ending_time_sampling < $time_min)
    {
      print "Error! Ending time is less than the minimum time $time_min (fs). Change ending time from                              $ctrl_ending_time_sampling to $time_min;\n";
      $ctrl_ending_time_sampling = $time_min;
      $tmp=int($time_min/$run_no_timestep);

      $j=0;
      while($dirs[$j] ne $tmp){$j++;}
      $sampling_indx_end_dir=$j;
    }#end elsif ($ctrl_ending_time_sampling < $time_min)
    else    #need do something  
    {
      if($_==0 and $ctrl_ending_time_sampling>0){$sampling_indx_end_dir--;}    #because index of array starts from 0
    }#end else of elsif($ctrl_ending_time_sampling < $time_min) of if($ctrl_ending_time_sampling > $time_max)

    print "final value= $sampling_indx_end_dir\t$dirs[$sampling_indx_end_dir]\n";#$tmp=<STDIN>;

###del     ### check data is in the limit or not
###del     if($sampling_indx_end_dir > $#dirs or $sampling_indx_start_dir < 0)
###del     { $flag=0;$error_msg="$error_msg sampling time range ($ctrl_starting_time_sampling-$ctrl_ending_time_sampling) is out of the limit(TIMESTEP:$timestep_max or time:$time_max(fs));";
###del       print "Sampling time range $ctrl_starting_time_sampling-$ctrl_ending_time_sampling is out of limits (TIMESTEP:$timestep_max or time:$time_max(fs)). Quit.\n";
###del       goto MAIN;
###del     }#end if($sampling_indx_end_dir > $#dirs or $sampling_indx_start_dir < 0)

#  END_SETTING_DIR_TIME:  

  
###dulplicated    SETTING_CN_QN_ANALYSIS:
###dulplicated    ### set the initial and final index for @dirs
###dulplicated    # here, for CN/Qn analysis, some/all elements in @dirs will be visited
###dulplicated    $sampling_indx_start_dir=int($sampling_start_timestep_no/$run_no_timestep);
###dulplicated    $_=$sampling_start_timestep_no%$run_no_timestep;
###dulplicated    if($_==0 and $ctrl_starting_time_sampling>0){$sampling_indx_start_dir--;}
###dulplicated    $sampling_indx_end_dir=int($sampling_end_timestep_no/$run_no_timestep);$_=$sampling_end_timestep_no%$run_no_timestep;
###dulplicated    if($_==0 and $ctrl_ending_time_sampling>0){$sampling_indx_end_dir--;}
    ...
    while($dir_loop_i<=$sampling_indx_end_dir)
    {
      ...
      ## the operation below will slow down the program
      ## however, the rest program need the @values
      ...
      ## reset variables for CN/Qn analysis with very limited frames
      if($flg_BL<0) #reset variables for CN/Qn analysis with very limited framesif($flg_BL<0)  
      {
        $cnt_atm_tot_intf=0;
        $cnt_atm_tot_exbox=0;
        $cnt_former_intf=0;
      ...
      }#end if($flg_BL<0) #reset variables for CN/Qn analysis with very limited framesif($flg_BL<0)  
      ...
      for ($j=$indx_line_no_ini_frm;$j<=$indx_line_no_last_frm;$j++)  #to extract one frame, the last frame is intentionally skipped because it is the same to the first frame except for timestep=0
      {
        ...
        system("sed -n \"$first_line_frm, $last_line_frm p\" $file >$tmp_file;");
        print "done\n";
    ...
    SETTING_BL_ANALYSIS:
      ...
      else                                #CN/Qn analysis
      {
        ...
        if($flg_interface<=0)    #surface analysis
        {
          ...
          {
#print "1109 \$sampling_start_timestep_no=$sampling_start_timestep_no\t\$sampling_end_timestep_no=$sampling_end_timestep_no\n\$ctrl_loop_tstp_sampling=$ctrl_loop_tstp_sampling\t\$ctrl_loop_max_time_sampling=$ctrl_loop_max_time_sampling\n";#$tmp=<STDIN>;
...
#        else{;}
      ...
      END_SETTING_BL_ANALYSIS:
        ...
        { $search_atom_id=1;
          ...
          else{;}   #user define the depth, recommended value 2
...
##   MAY del            else{$flg_c=0;} 
              ...
              $cn_no=&cn_distribution($line);chomp($cn_no);   #cn only for target atom 
...
#p  rint "1044 \$line=$line\n";    
    ...
    #  print colored("953 \$flg_former=$flg_former\t\$hash_former_cn_tot_amt{$cn_no}=$hash_former_cn_tot_amt{$cn_no}\n","bold yellow");$_=<STDIN>;
      ...
      #print  "foreach if \$line=$line\t";
              ...
              #count formers and oxygen grouped by slabs and CN
              if((grep {$tgt_nm eq $_ } @former_lists) or $tgt_nm = 'O')
              {
                #for(my $ss=int($lo_g);$ss<=(int($up_g)+1);$ss++) 
                if($zzz[$search_atom_id] >= $lo_g and $zzz[$search_atom_id] <= $up_g and $flg_intf_atm[$search_atom_id]=1)
                { $tmp=int($lo_g)+int($zzz[$search_atom_id] % int($lo_g))+0.5;  #decide atom in which slab range
                  $hash_key="$tgt_cn-$tgt_nm-$tmp";
                  $hash_no_former_cn_slb{$hash_key}++;
if(uc($tgt_nm) eq "O")
{print"1968: $search_atom_id - \$hash_key=$hash_key\t$hash_no_former_cn_slb{$hash_key}\n";<>;}
                }
              }
      ...
      OUTPUT_CN_QN:
        ...
        {
          if($flg_interface>0){$file_cn_qn="$work_dir/$ctrl_dir_trck/cb$comp_id\_CN_Qn_size_$sampling_size\_TMSTP_$frm_time\_$sampling_start_timestep_no-$sampling_end_timestep_no-$ctrl_loop_tstp_sampling-$ctrl_loop_max_time_sampling.dat";}
          else{$file_cn_qn="$work_dir/$ctrl_dir_trck/cs$comp_id\_CN_Qn_size_$sampling_size\_TMSTP_$frm_time\_$sampling_start_timestep_no-$sampling_end_timestep_no-$ctrl_loop_tstp_sampling-$ctrl_loop_max_time_sampling.dat";}
          ...
          if($#former_lists > 0)      
          {
            foreach $line (@former_lists)
            {
              ...
              if ($flg_BL<0){printf CN_QN "FRAM_$sn_time_step\t$frm_time(fs)\t";}
              printf CN_QN "$line        %6.0f  ABS_Qn  INTF_BOX  %6.0f  %6.0f  ",$sampling_size,$cnt_former_intf,$hash_former_intf_indv_amt{$line};
          ...
          if($flg_interface<0 or ($flg_interface>0 and $flg_vacuum<=0))   #surface analysis, there might be existing extended box
          {printf CN_QN "EXT_BOX  %6.0f  %6.0f  ",$cnt_oxyg_exbox,$oxy_wtr_exbox_cnt;
          for($cn_no=-1;$cn_no<=$ctrl_max_cn;$cn_no++){printf CN_QN "%7.3f ",$hash_oxygen_cn_water_exbox_tot_amt{$cn_no};}}
          print CN_QN "\n";
     
          ## NOTE: no hydrogen info output           
          #tmp skip ####### printf CN_QN "OXY_TOT   %6.0f  ABS_CN  INTF_BOX  %6.0f  %6.0f  ",$sampling_size,$cnt_atm_tot_intf,          $cnt_oxyg_intf;
          #tmp skip ####### for($cn_no=-1;$cn_no<=$ctrl_max_cn;$cn_no++){printf CN_QN "%7.3f ",$hash_oxygen_cn_tot_amt{$cn_no};}
          #tmp skip ####### if($flg_interface<0 or ($flg_interface>0 and $flg_vacuum<=0))   #surface analysis, there might be existing   extended box
          #tmp skip ####### {printf CN_QN "EXT_BOX  %6.0f  %6.0f  ",$cnt_atm_tot_exbox,$cnt_oxyg_exbox;
          #tmp skip ####### for($cn_no=-1;$cn_no<=$ctrl_max_cn;$cn_no++){printf CN_QN "%7.3f ",$hash_oxygen_cn_exbox_tot_amt{$cn_no};}}
          #tmp skip ####### print CN_QN "\n";
          ...
          if ($flg_BL<0){printf CN_QN "FRAM_$sn_time_step\t$frm_time(fs)\t";}
          printf CN_QN "FORMER    %6.0f  NORM_CN  INTF_BOX  %6.0f  %7.3f  ",$sampling_size,$cnt_atm_tot_intf/$sampling_size,100*$cnt_former_intf/$cnt_atm_tot_intf;
          ...
          if ($flg_BL<0){printf CN_QN "FRAM_$sn_time_step\t$frm_time(fs)\t";}
          printf CN_QN "FORMER    %6.0f  NORM_Qn  INTF_BOX  %6.0f  %6.0f  ",$sampling_size,$cnt_atm_tot_intf/$sampling_size,100*$cnt_former_intf/$cnt_atm_tot_intf;
          ...
          if($#former_lists > 0)      
          {
            ...
            {
              if ($flg_BL<0){printf CN_QN "FRAM_$sn_time_step\t$frm_time(fs)\t";}
              #printf CN_QN "$line       %6.0f  NORM_CN  INTF_BOX  %6.0f  %7.3f  ",$sampling_size,$cnt_atm_tot_intf,$cnt_former_intf;
              printf CN_QN "$line        %6.0f  NORM_CN  INTF_BOX  %6.0f  %7.3f  ",$sampling_size,$cnt_former_intf/$sampling_size,100*$hash_former_intf_indv_amt{$line}/$cnt_former_intf;
              if($hash_former_intf_indv_amt{$line}==0){$warn_msg="$warn_msg no former in interface box \$hash_former_intf_indv_amt{$line}=$hash_former_intf_indv_amt{$line};";$hash_former_intf_indv_amt{$line}=-1;}   #if species does not exist, given -1 in case of illegal division
              ...
              if($hash_former_exbox_indv_amt{$line}==0){$hash_former_exbox_indv_amt{$line}=-1;}   #if species does not exist, given -1 in case of illegal division
              ...
              if ($flg_BL<0){printf CN_QN "FRAM_$sn_time_step\t$frm_time(fs)\t";}
              printf CN_QN "$line        %6.0f  NORM_Qn  INTF_BOX  %6.0f  %7.3f  ",$sampling_size,$cnt_former_intf/$sampling_size,100*$hash_former_intf_indv_amt{$line}/$cnt_former_intf;
          ...
          }#end if($#former_lists > 0)
          else{;} 
          ## output CN for modifier
          if($cnt_modf_intf==0){$cnt_modf_intf=1;}
          if($cnt_former_intf==0){$cnt_former_intf=1;}
          if ($flg_BL<0){printf CN_QN "FRAM_$sn_time_step\t$frm_time(fs)\t";}
          printf CN_QN "MODIFER   %6.0f  NORM_CN  INTF_BOX  %6.0f  %7.3f  ",$sampling_size,$cnt_atm_tot_intf/$sampling_size,100*$cnt_modf_intf/$cnt_atm_tot_intf;
          ...
          if ($flg_BL<0){printf CN_QN "FRAM_$sn_time_step\t$frm_time(fs)\t";}
          printf CN_QN "OXY_TOT   %6.0f  NORM_CN  INTF_BOX  %6.0f  %7.3f  ",$sampling_size,$cnt_atm_tot_intf/$sampling_size,100*$cnt_oxyg_intf/$cnt_atm_tot_intf;
          ...
          if ($flg_BL<0){printf CN_QN "FRAM_$sn_time_step\t$frm_time(fs)\t";}
          printf CN_QN "OXY_GLS   %6.0f  NORM_CN  INTF_BOX  %6.0f  %7.3f  ",$sampling_size,$cnt_oxyg_intf/$sampling_size,100*$oxy_gls_intf_cnt/$cnt_oxyg_intf;
          ...
          if ($flg_BL<0){printf CN_QN "FRAM_$sn_time_step\t$frm_time(fs)\t";}
          printf CN_QN "OXY_WAT   %6.0f  NORM_CN  INTF_BOX  %6.0f  %7.3f  ",$sampling_size,$cnt_oxyg_intf/$sampling_size,100*$oxy_wtr_intf_cnt/$cnt_oxyg_intf;
